...
class SESEmailSender:
    ...
    def __init__(self, region: str = "us-east-1", sender_email: Optional[str] = None):
        self.region = region
    ...
    def validate_credentials(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"SES credentials valid. Quota: {sent_24hr:.0f}/{max_24hr:.0f} emails (24hr)"
            )
            return True
        except ClientError as e:
            logger.error(
                f"SES credential validation failed: {e.response['Error']['Code']}"
            )
            return False
        except Exception as e:
    ...
    def check_email_verified(self, email: str) -> bool:
        ...
        try:
            ...
            status = attributes.get(email, {}).get("VerificationStatus", "NotVerified")
