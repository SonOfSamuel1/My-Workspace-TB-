...
class YNABService:
    ...
    def update_transaction_memo(
        ...
        if items:
            # Build detailed memo from items
            item_details = []
            for item in items[:3]:  # Limit to first 3 items to avoid memo length issues
                item_name = item.get('name', 'Unknown Item')[:50]  # Truncate long names
                item_qty = item.get('quantity', 1)
                item_category = item.get('category', 'Shopping')
            ...
            # Format the memo
            if order_id:
                new_memo = f"[Amazon #{order_id[-6:]}] "  # Use last 6 chars of order ID
            else:
                new_memo = "[Amazon] "

            new_memo += f"{', '.join(item_details)}"

            # Add category if single category across all items
            categories = list(set(item.get('category', 'Shopping') for item in items))
            if len(categories) == 1 and categories[0] != 'Shopping':
                new_memo += f" | {categories[0]}"

            # Add item count if more than shown
            if len(items) > 3:
                new_memo += f" (+{len(items)-3} more)"
        else:
