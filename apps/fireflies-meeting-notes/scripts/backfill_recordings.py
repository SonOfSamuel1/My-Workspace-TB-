...
Regenerates index.json at the end for fast dashboard loads.
...
def main():
    ...
    # Rebuild index.json for fast dashboard loads
    if processed > 0 or skipped > 0:
        logger.info("Rebuilding index.json...")
        count = build_index(s3_bucket)
        logger.info(f"index.json rebuilt with {count} recordings")
