...
class YNABService:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        if not self.api_key:
            raise ValueError(
                "YNAB API key is required. Set YNAB_API_KEY environment variable."
            )
    ...
    def get_annual_transactions(self, budget_id: str, year: int) -> List[Dict]:
        ...
        """
        self.logger.info(
            f"Fetching annual transactions for {budget_id}, year {year}..."
        )
        ...
        year_transactions = [
            txn for txn in transactions if txn["date"].startswith(str(year))
        ]

        self.logger.info(f"Found {len(year_transactions)} transactions for {year}")
        return year_transactions

    def get_monthly_budgets(self, budget_id: str, year: int) -> Dict[str, Dict]:
    ...
    def aggregate_annual_budget(
        self, monthly_budgets: Dict[str, Dict]
    ) -> Dict[str, Dict]:
            ...
            for category in month_data.get("categories", []):
                ...
                if cat_id not in annual_budget:
                    annual_budget[cat_id] = {
                        ...
                        "annual_activity": 0,
                        "monthly_breakdown": {
                            m: {"budgeted": 0, "activity": 0} for m in month_names
                        },
                    }
        ...
        self.logger.info(
            f"Aggregated annual budget for {len(annual_budget)} categories"
        )
