#!/usr/bin/env python3
"""
Main entry point for Relationship Tracking Automation

This script orchestrates the Love Brittany Action Plan tracking system,
generating comprehensive reports on relationship activities.
"""
...
def setup_logging(config: dict):
    ...
    log_file = log_config.get('file', 'logs/relationship.log')
...
def validate_configuration(config: dict) -> bool:
    ...
    # Check relationship tracking enabled
    if not config.get('relationship_tracking', {}).get('enabled'):
        logger.warning("Relationship tracking is disabled in config.yaml")
        return False

    # Check tracking document ID
    tracking_config = config.get('relationship_tracking', {})
    if not tracking_config.get('tracking_doc_id') and not os.getenv('RELATIONSHIP_TRACKING_DOC_ID'):
        logger.error("Tracking document ID not configured")
        logger.error("Set 'tracking_doc_id' in config.yaml or RELATIONSHIP_TRACKING_DOC_ID in .env")
        return False
...
def generate_report(config: dict, send_email: bool = True):
    """
    Generate and optionally send relationship tracking report.
    ...
    try:
        ...
        tracking_config = config['relationship_tracking']

        # Override with environment variables if present
        tracking_config['tracking_doc_id'] = os.getenv(
            'RELATIONSHIP_TRACKING_DOC_ID',
            tracking_config.get('tracking_doc_id')
        )

        tracking_config['toggl_project_name'] = os.getenv(
            'TOGGL_RELATIONSHIP_PROJECT',
            tracking_config.get('toggl_project_name', 'Love Brittany')
        )

        # Create tracker
        logger.info("Creating Relationship Tracker...")
        ...
        logger.info("Generating relationship report data...")
        report_data = tracker.generate_report()

        # Generate HTML report
        logger.info("Generating HTML email...")
        report_generator = RelationshipReportGenerator(tracking_config)
        ...
        if send_email:
            recipient = os.getenv(
                'RELATIONSHIP_REPORT_EMAIL',
                tracking_config.get('email', {}).get('recipient')
            )
            ...
            subject_template = tracking_config.get('email', {}).get(
                'subject_template',
                'Love Brittany Action Plan Report - {date}'
            )
        ...
        print(f"📅 Date Nights Scheduled: {report_data['date_nights']['total_scheduled']}")
        print(f"⏱️  Time Investment (30d): {report_data['toggl_stats']['total_hours']:.1f}h")
...
def validate_setup():
    ...
    print("\n" + "="*60)
    print("RELATIONSHIP TRACKING SETUP VALIDATION")
    print("="*60 + "\n")
    ...
    try:
        ...
        doc_id = os.getenv('RELATIONSHIP_TRACKING_DOC_ID', config['relationship_tracking'].get('tracking_doc_id'))
        if doc_id:
            try:
        ...
        print("Next steps:")
        print("1. Create your tracking document with required sections")
        print("2. Run: python src/relationship_main.py --generate")
        print("3. Or set up scheduler for automatic reports\n")
...
def main():
    ...
    logger = logging.getLogger(__name__)
    logger.info("="*60)
    logger.info("Starting Relationship Tracking Automation")
    logger.info("="*60)
    ...
    if args.validate:
        ...
        parser.print_help()
        print("\nExamples:")
        print("  python src/relationship_main.py --validate")
        print("  python src/relationship_main.py --generate")
        print("  python src/relationship_main.py --generate --no-email")
